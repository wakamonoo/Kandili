...
import {
  ...
  Toast,
  escapeHTML,
} from "./ui.js"; // Assuming ui.js has these
...
export function setupNewsfeedListeners() {
  ...
  if (!DOM.allPostsTab || !DOM.latestPostsTab || !DOM.newsfeedContent) {
    console.error(
      "Newsfeed DOM elements (allPostsTab, latestPostsTab, newsfeedContent) not found. Aborting newsfeed setup."
    );
...
function setActiveTab(activeId) {
  ...
  allTabs.forEach((tab) => {
    if (tab) {
      // Defensive check, though should be present due to earlier check
...
function renderNewsfeedPost(entry) {
  ...
  postElement.innerHTML = `
        <div class="flex items-center mb-3">
            <img src="${ownerPhoto}" alt="${escapeHTML(
    ownerName
      ...
      try {
        await db
          ...
          .add({
            ...
            userPhoto:
              auth.currentUser.photoURL || "https://via.placeholder.com/24",
        ...
        (snap) => {
          ...
          snap.forEach((commentDoc) => {
            ...
            commentDiv.innerHTML = `
                        <img src="${
                          c.userPhoto || "https://via.placeholder.com/24"
                        }" alt="${escapeHTML(
              c.userName
            )}" class="w-6 h-6 rounded-full mr-2 mt-1">
                        <div>
                            <p><span class="font-semibold">${escapeHTML(
                              c.userName
                            )}:</span> ${escapeHTML(c.commentText)}</p>
